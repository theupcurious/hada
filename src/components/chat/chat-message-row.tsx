...
function UserMessageContent({ content }: { content: string }) {
  return (
    <div className="min-w-0 w-full max-w-full overflow-hidden text-base leading-relaxed space-y-1 [overflow-wrap:anywhere] [&>*]:min-w-0 [&>*:last-child]:mb-0">
      <ReactMarkdown
        ...
        components={{
          ...
          h1: ({ children }) => (
            <p className="mt-3 mb-2 text-lg font-bold text-zinc-900 first:mt-0 dark:text-zinc-100">{children}</p>
          ),
          h2: ({ children }) => (
            <p className="mt-3 mb-1.5 text-base font-bold text-zinc-900 first:mt-0 dark:text-zinc-100">{children}</p>
          ),
          h3: ({ children }) => (
          ...
          pre: ({ children }) => (
            <pre className="mb-2 max-w-full overflow-x-auto rounded-lg bg-zinc-100 p-3 font-mono text-sm whitespace-pre dark:bg-zinc-800">
              {children}
            </pre>
          ),
          code: ({ className, children }) => {
            ...
            return isBlock ? (
              ...
              <code className="break-all rounded bg-zinc-100 px-1 py-0.5 font-mono text-sm dark:bg-zinc-800">
                {children}
              </code>
            );
          },
          ...
          th: ({ children }) => (
            <th className="border-b border-zinc-200 bg-zinc-50 px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider break-words text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900/60 dark:text-zinc-400">
              {children}
            </th>
          ),
          td: ({ children }) => (
