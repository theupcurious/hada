...
export default function ActivityPage() {
  ...
  const load = useCallback(
    async (offset: number) => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/dashboard/activity?limit=${PAGE_SIZE}&offset=${offset}`, {
          cache: "no-store",
        });
        if (res.status === 401) {
...
function ActivityRow({ run }: { run: AgentRun }) {
  ...
  return (
    <li className="rounded-2xl border border-zinc-200 bg-white/80 p-4 dark:border-zinc-800 dark:bg-zinc-950/50">
      ...
      {run.output_preview && run.output_preview.trim() ? (
      ...
      {run.error && run.error.trim() ? (
        <p className="mt-2 text-xs leading-5 text-red-600 dark:text-red-400">{run.error}</p>
      ) : null}
    </li>
  );
}
...
function StatusIcon({ status }: { status: AgentRun["status"] }) {
  if (status === "completed") {
    return <CheckCircle2 className="h-4 w-4 shrink-0 text-teal-500" aria-label="Completed" />;
  }
  if (status === "running") {
    return <Clock className="h-4 w-4 shrink-0 animate-pulse text-amber-500" aria-label="Running" />;
  }
  return <XCircle className="h-4 w-4 shrink-0 text-red-500" aria-label={status} />;
}
