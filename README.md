...
- `TELEGRAM_BOT_TOKEN`, `TELEGRAM_BOT_USERNAME`, `TELEGRAM_WEBHOOK_SECRET` (required if the bot is enabled — the webhook fails closed)
- `INTEGRATION_ENCRYPTION_KEY` (required for Google — encrypts stored OAuth tokens; `openssl rand -base64 32`)
- `SEARCH_PROVIDER`, `SEARCH_API_KEY`, `TAVILY_API_KEY`, `BRAVE_API_KEY`, `SERPAPI_API_KEY`
- `CRON_SECRET` (required — `/api/cron` fails closed)
...
24. `024_rate_limits_and_share_expiry.sql`
...
- Tool permissions are enforced by `DEFAULT_POLICY` in `src/lib/chat/tool-permissions.ts` (per-source via `policyForSource`). External tool output is wrapped as untrusted content; see `docs/SECURITY-REVIEW.md`.
